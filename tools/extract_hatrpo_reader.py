...
OUT_DIR = PDF_PATH.parent / "P0_01_2021_HATRPO_HAPPO_Trust_Region_MARL_reader"


def paragraph_blocks(text):
    section_re = re.compile(
        r"^(?:[0-9]+(?:\.[0-9]+)*\s+|[A-Z]\.\d*\s+|APPENDIX|REFERENCES|ABSTRACT|ACKNOWLEDGMENTS)"
    )
    paras = []
    cur = []
    for raw_line in text.splitlines():
        ...
        if section_re.match(line) and cur:
            ...
            cur = []
        if len(line) < 64 and (section_re.match(line) or line.isupper()):
            if cur:
                paras.append(" ".join(cur))
                cur = []
            paras.append(line)
