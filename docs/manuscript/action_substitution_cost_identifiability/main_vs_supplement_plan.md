...
| MAIN-03 | 三类替代成本与主恒等式 | §4 | Fig. 2 | 核心方法模块 |
| MAIN-04 | R1 的正替代和非正成本解释 | §6.1 | Fig. 2 | 建立测量混合的直接证据 |
| MAIN-05 | future-only 遗漏与完整账本修正 | §6.2 | Fig. 2 | 证明同一步后缀项不可省略 |
| MAIN-06 | 新模型、新上下文和 Actor 冻结完整性 | §5、§6.3 | Table 1 | 支撑“独立确认” |
| MAIN-07 | seeds 17/18/19 块级复现 | §6.3 | Fig. 3 | 支撑 C3 |
| MAIN-08 | 场景与资源类型边界 | §6.4 | Fig. 4 | C4 否决是核心科学边界 |
| MAIN-09 | 资源恢复通用机会价值失败 | §6.5 | Table 2 | 解释为何不进入 opportunity oracle |
| MAIN-10 | 在线 BPCE/MCH-PPO 总门控失败 | §8 | Table 2 | 防止把测量贡献误写为算法胜出；不占用 Results 6.6 |
...
| Fig. 2 | 局部成本混合、三分量恒等式和账本完整性 | C1、C2 | 跨环境普遍性 |
| Fig. 3 | 新策略种子独立确认 | C3 | 分布外泛化 |
| Fig. 4 | 场景和资源类型 \(\rho_{\mathrm{sub}}\) 与符号边界 | C4 | 所有资源类型同强度 |
| Table 1 | R2 独立性、统计单位和完整性 | C2、C3 | 把 ledger row 当独立样本 |
| Table 2 | C4-C6 负边界与门控结果 | C4-C6 | 将失败包装为算法优势 |
