...
工程 v0 不应直接引入新的可学习乘子、Critic 或复杂最优传输。更稳妥的实现是：

- 对 rollout 中的早期前缀，固定旧策略的部署动作或实际采样动作 \(a^{\mathrm{old}}\)；
- 计算新策略分配给候选动作 \(b\) 的概率质量；
- 用 \(c_{\mathrm{DS}}(a^{\mathrm{old}},b)\) 加权得到 support-disruptive probability mass；
