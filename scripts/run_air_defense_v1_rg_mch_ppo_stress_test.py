...
DEFAULT_OUTPUT = (
    ...
    / "results"
    / "air_defense_v1"
    / "rg_mch_ppo_mechanism_stress_test"
)
DEFAULT_REFERENCE = (
    PROJECT_ROOT
    / "results"
...
CANDIDATE = "rg_mch_ppo_order_012"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the preregistered RG-MCH-PPO mechanism stress test."
    )
...
def _summary(
    ...
    rows = tuple(candidate_rows) + tuple(reference_rows)
    on_scenario = [
    ...
    for scenario in scenarios:
        ...
        for seed in seeds:
            ...
            collapsed_count += int(candidate_collapsed)
            ...
            paired.append(
                {
                    ...
                    "reward_delta_vs_mch_v0": _number(candidate, "avg_reward")
                    - _number(mch_v0, "avg_reward"),
                    "damage_delta_vs_mch_v0": _number(
                        candidate, "avg_total_damage"
                    )
                    - _number(mch_v0, "avg_total_damage"),
                }
            )

        noop_noninferior_count = sum(
        ...
        scenario_noop_passed = noop_noninferior_count >= max(1, len(seeds) - 1)
        scenario_safety_passed = reward_delta >= -10.0 and damage_delta <= 0.20
        scenario_cost_passed = cost_ratio <= 1.10
        scenario_mch_passed = reward_delta_mch > 0.0 and damage_delta_mch < 0.0
        all_noop_noninferior = all_noop_noninferior and scenario_noop_passed
        safety_noninferior = safety_noninferior and scenario_safety_passed
        resource_cost_passed = resource_cost_passed and scenario_cost_passed
        improves_high_threat = improves_high_threat or high_threat_delta < 0.0
        improves_mch_v0 = improves_mch_v0 and scenario_mch_passed
        scenario_records[scenario] = {
            "paired_runs": paired,
            "all_noop_noninferior_seed_count": noop_noninferior_count,
            "all_noop_gate_passed": scenario_noop_passed,
            ...
            "mch_v0_improvement_gate_passed": scenario_mch_passed,
        }
    ...
    gates = {
        ...
        "improves_mch_v0_both_scenarios": improves_mch_v0,
    }
    return {
        "schema_version": 1,
        "methods": [BASELINE, MCH_V0, CANDIDATE],
    ...
    for scenario, record in summary["scenarios"].items():
        lines.extend(
            [
                ...
                f"- 奖励差 vs MCH v0：{record['mean_reward_delta_vs_mch_v0']:.6f}",
                f"- 损伤差 vs MCH v0：{record['mean_damage_delta_vs_mch_v0']:.6f}",
                "",
...
def _write_summary(output_dir: Path, summary: dict[str, Any]) -> None:
    (output_dir / "rg_mch_stress_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    (output_dir / "rg_mch_stress_summary.md").write_text(
